#repository = project
#cau hinh name va email: git config --global user.email (email)
#tao git local repo client (thu muc goc du an): git init
#=> tao repo tren may de link toi repo tren server
#ket noi voi server: git remote add origin (duong dan project)
    # kiem tra git remote -v => ra nguon goc du an
#dua tai lieu len remote server (lam thuong xuyen)
#git add (ten file, folder, tat ca (-A)): chon cac file dua len server
#=> kiem tra: git status
#git commit -m (ghi chu): backup version tren local (co the quay lai trang thai nay luc sau)
#day len server: git push -u origin (ten nhanh): dua cac commit tu client repo len server repo
#git push --set-upstream origin master: set up nhánh tren server neu chua co nhanh
#git clone => clone xuong local
#git pull => update version mới nhat tu server






#### deploy streamlit => streamlit.io
